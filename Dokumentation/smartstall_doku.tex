...
\section{Überblick}
In dieser Dokumentation wird zunächst die Anforderungsanalyse behandelt. Diese umfasst die ursprünglichen Überlegungen, die zu Beginn des Projekts gemacht wurden. Anschließend wurde das Konzept zwei Landwirten vorgestellt, deren Feedback in das endgültige Projekt einfloss.

Kapitel 2 erläutert die Hardware des Projekts und erklärt, wie der Raspberry Pi mit den Sensoren verkabelt wurde. Im weiteren Verlauf wird die Einrichtung des Raspberry Pi detailliert beschrieben.

Das Hauptkapitel 6 befasst sich mit der Webanwendung. Dabei werden die Architektur, das Hosting, das Deployment, das Software-Design, die Anbindung an die Datenbank, der Aufbau der Datenbanktabelle, die Schnittstellen zum Empfangen und Speichern der Sensordaten, das Auslesen und Anzeigen der Sensordaten, die Benutzeroberfläche, die Telegram-Anbindung und das Steuern der WLAN-Steckdose thematisiert.

Das vorletzte Kapitel beschreibt die Durchführung eines lokalen und eines produktiven Tests für das Gesamtsystem. Die Dokumentation schließt mit einer Zusammenfassung und einem Ausblick.
